...
Long, low-curvature spans take official straights on B–E. Curves are
banned on those spans so oval sides cannot sawtooth. Rescue keeps the lap
from dying.
"""
...
WIGGLE_SKUS = {"C8235", "C8206", "C8204", "C8234"}
STRAIGHT_SKUS = {"C8205", "C8207", "C8200", "C8236"}
...
def _heading_window(cl, s_idx, pose, window_mm: float) -> float:
    k = s_idx
    while k < len(cl.s) - 1 and cl.s[k] < cl.s[s_idx] + window_mm:
        k += 1
    return normalize_heading(cl.heading(min(k, len(cl.points) - 2)) - pose.heading_degrees)


def _max_heading_wobble(cl, s_idx, window_mm: float) -> float:
    end = s_idx
    while end < len(cl.s) - 1 and cl.s[end] < cl.s[s_idx] + window_mm:
        end += 1
    h0 = cl.heading(s_idx)
    peak = 0.0
    step = max(1, (end - s_idx) // 12)
    for i in range(s_idx, min(end + 1, len(cl.points) - 1), step):
        peak = max(peak, abs(normalize_heading(cl.heading(i) - h0)))
    return peak
...
def sequential_follow(
    ...
    **_kwargs,
    ...
    while s_idx < len(cl.points) - 8 and len(seq) < max_pieces:
        turn_needed = _heading_window(cl, s_idx, pose, look_ahead_mm)
        long_turn = _heading_window(cl, s_idx, pose, 550.0)
        far_turn = _heading_window(cl, s_idx, pose, 900.0)
        wobble = _max_heading_wobble(cl, s_idx, 700.0)
        almost_straight = (
            abs(turn_needed) < 20.0
            and abs(long_turn) < 28.0
            and wobble < 32.0
            and not _in_span(spans, s_idx)
        )
        lock_straights = almost_straight and abs(far_turn) < 36.0
        hp_turn = _in_span(spans, s_idx)
        spiral = _recent_spiral(seq, get_part)
        wig = _wiggle_sign(seq, get_part)
        last_was_straight = bool(seq) and _root(seq[-1]) in STRAIGHT_SKUS
        hairpin_run = 0
        ...
        def _score_pool(pool, *, rescue=False):
            ...
            for code in pool:
                ...
                if is_straight and not rescue:
                    if (hp_turn or abs(turn_needed) > 38) and part.geometry.length >= 250:
                        continue
                if isinstance(part.geometry, CurveGeometry) and not rescue:
                    if (almost_straight or lock_straights) and not hp_turn:
                        continue
                    if abs(turn_needed) < 18 and _root(code) in WIGGLE_SKUS and not hp_turn:
                        continue
                    if abs(turn_needed) < 16 and abs(signed) >= 20 and not hp_turn:
                        continue
                    ...
                    if is_hp and hairpin_run >= 3:
                        continue
                if lock_straights and last_was_straight and not is_straight and not rescue:
                    continue
                ...
                if is_straight:
                    tol += 140 if almost_straight else 80
                if ndist > tol:
                    continue
                if no_chord and not rescue:
                    mx, my = (pose.x + np.x) * 0.5, (pose.y + np.y) * 0.5
                    _, mid_d = cl.closest(mx, my, start=max(0, s_idx - 1), window=win)
                    mid_lim = dist_tol_mm * (1.55 if is_straight and almost_straight else (1.25 if is_straight else 0.95))
                    if mid_d > mid_lim:
                        continue
                if is_straight:
                ...
                kink = abs(normalize_heading(np.heading_degrees - pose.heading_degrees)) if is_straight else 0.0
                sc = ndist * 5.0 + head_err * 3.0 - prog * 0.5 + kink * 12.0
                if hp_turn and is_hp:
                    sc -= 55.0
                if is_straight and abs(turn_needed) < 22:
        ...
        pool = codes
        if lock_straights:
            only = [c for c in longs if c in codes] or [c for c in codes if _root(c) in STRAIGHT_SKUS]
            if only:
                pool = only
        best = _score_pool(pool)
        if best is None and lock_straights:
