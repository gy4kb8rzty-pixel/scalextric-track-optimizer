...
BUDGET_CANDIDATES = ["C8207", "C8200", "C8236", "C8206L", "C8206R", "C8010L", "C8010R", "C8204L", "C8204R", "C8235L", "C8235R"]
...
LEVELS = {
    ...
    AccuracyLevel.LEAN_BUDGET: _p(AccuracyLevel.LEAN_BUDGET, "A", "Lean Budget", "Follow the red line with as few pieces as possible.", "sequential", False, False, 56, 8, 9000.0, 36.0, 32.0, 28.0, 900.0, 280.0, 400.0, 140, True, False, False, "lean", scale_frac=0.30, min_target_mm=7500.0, max_target_mm=11000.0),
    AccuracyLevel.BUDGET: _p(AccuracyLevel.BUDGET, "B", "Budget", "Smaller red guide. Follow the silhouette, no long chords.", "sequential", False, False, 40, 8, 11000.0, 36.0, 40.0, 36.0, 480.0, 180.0, 260.0, 160, False, False, False, "budget", scale_frac=0.36, min_target_mm=8500.0, max_target_mm=13000.0),
...
def levels_for_ui(track_id: str | None = None):
    from monza_optimizer.reference.tracks import LEVEL_A_TRACKS, level_a_allowed
    rows = []
    for p in LEVELS.values():
        a_ok = True if p.letter != "A" else (level_a_allowed(track_id) if track_id else False)
        rows.append({
            "id": p.level.value,
            "letter": p.letter,
            "label": p.label,
            ...
            "restricted_to_tracks": sorted(LEVEL_A_TRACKS) if p.letter == "A" else None,
        })
    return rows
