...
def closure_metrics(seq: list[str], start: Pose, get_part: Callable) -> dict:
    if not seq:
        return {"pos_mm": float("inf"), "head_deg": 180.0, "closed": False}
...
def close_loop(
    ...
    *,
    ...
    if not seq:
        pos, head = float("inf"), 180.0
    else:
        pos = math.hypot(pose.x - start.x, pose.y - start.y)
        head = abs(normalize_heading(pose.heading_degrees - start.heading_degrees))
    stats = {"pos_before_mm": pos if pos != float("inf") else 0.0, "head_before_deg": head, "added": []}
    if seq and pos <= CLOSE_POS_MM and head <= CLOSE_HEAD_DEG:
    ...
    for _ in range(max_pieces):
        bp = best[2]
        if seq and math.hypot(bp.x - start.x, bp.y - start.y) <= CLOSE_POS_MM and abs(
        ...
        beam = nxt[:beam_width]
        if beam[0][0] < best[0] or not (seq or best[1]):
            best = beam[0]
    extra = best[1]
    end = best[2]
    pos2 = math.hypot(end.x - start.x, end.y - start.y)
    head2 = abs(normalize_heading(end.heading_degrees - start.heading_degrees))
    closed = bool(seq or extra) and pos2 <= CLOSE_POS_MM and head2 <= CLOSE_HEAD_DEG
    stats.update({"added": extra, "pos_mm": pos2, "head_deg": head2, "closed": closed})
